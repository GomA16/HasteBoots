...
impl<F: Field + Serialize> NTTFourierEvalIOP<F> {
    ...
    ///
    /// # Arguments
    /// * num_vars: denoted by k, which is also the round number iterated in a reverse order as described in the algorithm
    /// * requested_point: the random point $(x, b)\in \mathbb{F}^{k+1}$ reduced from the last sumcheck, used to prove the sum in the round
    ...
    pub fn delegation_proof_iteration(
        ...
        let eq_func = gen_identity_evaluations(requested_point);
    ...
    pub fn delegation_verify_round(
        ...
        let log_n = ntt_instance_info.log_coeff_count;
