...
use piop::lookup::indexed_table::{BatchedIndexLogUpInputProof, IndexedLogUpInputIOP, IndexedLogUpInputInstance, IndexedLogUpInputProof};
...
use piop::{
    ...
    SumcheckPIOP, SumcheckPureBatchedProof,
};
use serde::Serialize;
        ...
        >,
{
    ...
    pub fn prove_as_subprotocol(
        ...
        let lookup_trace = trace_ef.extract_indexed_lookup_trace_mle(params.basis);
        let lookup_witness = lookup_trace
            .iter()
            .map(|trace| trace.compute_witness())
            .collect::<Vec<_>>();
        let lookup_helper = lookup_trace
            .iter()
            .zip(lookup_witness.iter())
            .map(|(trace, witness)| {
                trace.compute_helper_functions(witness, random_value, random_s_hash)
            })
            .collect::<Vec<_>>();
        ...
        // PIOP Phase: Hadamard + NTT Matrix Eval
        let time = std::time::Instant::now();
        let hadamard_instance = SumHadamardInstance::from(&trace_ef);
        let lookup_instance = lookup_trace
            .iter()
            .zip(lookup_helper.iter())
            .map(|(trace, helper)| IndexedLogUpInputInstance::from(trace, helper))
            .collect::<Vec<_>>();
        let hadamard_instance_info = hadamard_instance
            .iter()
            .map(SumcheckInstance::info)
            .collect::<Vec<_>>();
        let lookup_instance_info = lookup_instance
            .iter()
        ...
        assert_eq!(
            ...
            lookup_instance_info[0].sumcheck_num_vars()
        );
        ...
        let randomness_lookup = lookup_instance_info[0]
            .sample_randomness_for_sumcheck_batch(trans, lookup_instance.len());
        IndexedLogUpInputIOP::prover_batch_add_sumcheck(
            &lookup_instance,
            &mut sumcheck_claim,
            &randomness_lookup,
            lagrange_kernel,
        ...
        let trace_evals = trace_mle.evaluate_ef_with_lookup(
            &sumcheck_state.randomness,
            &trace_ef,
            &sumcheck_claim.poly,
            &eval_table,
        );
        let helper_evals = lookup_helper.iter().map(|helper| {
            sumcheck_claim.poly.lookup_mle_eval(&helper.helper_input, &eval_table, &sumcheck_state.randomness)
        }).collect::<Vec<_>>();
        let phi_evals = lookup_helper.iter().map(|helper| {
            sumcheck_claim.poly.lookup_mle_eval(&helper.phi_input, &eval_table, &sumcheck_state.randomness)
        }).collect::<Vec<_>>();

        let hadamard_eval_proof = BatchedSumHadamardProof::from_hadamard_trace_eval(&trace_evals);
        let mut lookup_eval_proof = BatchedIndexLogUpInputProof::<EF>::default();
        lookup_eval_proof.helper_input_at_rx = helper_evals;
        lookup_eval_proof.phi_input_at_rx = phi_evals;
