    ...
    PCS: PolynomialCommitmentScheme<F, EF, S>,
{
    pub fn new(code_spec: S, blk_size: usize, trace: &LookupTraceMLE<F>) -> Self {
        Self {
            // code_spec,
            blk_size,
            pcs_params: PCS::setup(trace.witness_num_vars(), &code_spec),
            pcs_params_ef: PCS::setup(trace.helper_num_vars(blk_size), &code_spec),
        }
    }
}
    ...
    PCS: PolynomialCommitmentScheme<F, EF, S>,
{
    ...
    // these redundant fields are for easier access
    pub log_num_oracles: usize,
    pub log_num_oracles_helper: usize,
    pub witness_eval: LookupWitnessEval<EF>,
        ...
        >,
{
    ...
    pub fn compute_oracle_evaluation(&self, eval: &LookupWitnessEval<EF>, point: &[EF]) -> EF {
    ...
    pub fn prove(
        ...
        let witness_eval = witness.evaluate_ef(&piop_state.point_r);
        let helper_eval = helper.evaluate(&piop_state.point_r);
        piop_proof.append_eval(&witness_eval, &helper_eval, random_value);
        ...
        let oracle_evaluation = self.compute_oracle_evaluation(&witness_eval, &point_trace_oracle);
        ...
        LogUpSnarksProof {
            ...
            eval_trace_proof,
            eval_helper_proof,
            log_num_oracles: witness.log_num_oracles(),
            log_num_oracles_helper: helper.log_num_oracles(),
            witness_eval,
            helper_eval,
        }
    }
    ...
    pub fn verifier(
        ...
        let point_helper_oracle = trans.get_vec_challenge(
            b"[Challenge] random point used to verify evaluations",
            proof.log_num_oracles_helper,
        );

        let oracle_evaluation =
            self.compute_oracle_evaluation(&proof.witness_eval, &point_trace_oracle);
        trans.append_message(b"[PIOP Phase]", &oracle_evaluation);
        let oracle_evaluation_ef =
