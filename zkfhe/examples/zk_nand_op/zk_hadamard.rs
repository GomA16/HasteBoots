...
use snarks::fhe_op::hadamard_old::{HadamardParams, HadamardSnarks};
...
fn main() {
    ...
    let ntt_table = FF::get_ntt_table(hadamard_trace.log_coeff_count as u32)
        .unwrap()
        .root_powers();
    let ntt_table = Rc::new(ntt_table.to_ef());

    let code_spec = ExpanderCodeSpec::new(0.1195, 0.0248, 1.9, BASE_FIELD_BITS, 10);
    let params = HadamardParams::new(code_spec.clone(), &ntt_table, &hadamard_trace);
    ...
    let proof = snarks.prove(&mut prover_trans, hadamard_trace, &params);
