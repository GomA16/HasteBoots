...
use crate::{FHECoreError, LWESecretKeyType, RingSecretKeyType};

/// Indicate whether to perform a `modulus switch`, `scale` or `noop` before blind rotation.
#[derive(Debug, Clone, Copy)]
pub struct ProcessBeforeBlindRotation<Q: NTTField> {
    lut_step: usize,
    /// `2N`
    twice_ring_dimension_value: Q::Value,
    twice_ring_dimension_modulus: PowOf2Modulus<Q::Value>,
}

impl<Q: NTTField> ProcessBeforeBlindRotation<Q> {
    /// Returns twice ring dimension value of this [`ProcessBeforeBlindRotation<C>`].
    pub fn twice_ring_dimension_value(&self) -> Q::Value {
        self.twice_ring_dimension_value
    }

    /// Returns twice ring dimension modulus of this [`ProcessBeforeBlindRotation<C>`].
    pub fn twice_ring_dimension_modulus(&self) -> PowOf2Modulus<Q::Value> {
...
pub struct Parameters<Q: NTTField> {
    lwe_params: LWEParameters<Q>,
    blind_rotation_params: BlindRotationParameters<Q>,
    key_switching_params: KeySwitchingParameters,
    process_before_blind_rotation: ProcessBeforeBlindRotation<Q>,
}
...
#[derive(Debug, Clone, Copy)]
pub struct ConstParameters<Q> {
    /// **LWE** vector dimension, refers to **n** in the paper.
    pub lwe_dimension: usize,
    /// **LWE** message modulus, refers to **t** in the paper.
    pub lwe_plain_modulus: Q,
...
impl<Q: NTTField> Parameters<Q> {
    /// Create a new Parameter instance.
    pub fn new(params: ConstParameters<Q::Value>) -> Result<Self, FHECoreError> {
        ...
        let process_before_blind_rotation = ProcessBeforeBlindRotation {
            lut_step: 1,
            twice_ring_dimension_value: twice_ring_dimension.as_into(),
            twice_ring_dimension_modulus: PowOf2Modulus::from(twice_ring_dimension.as_into()),
        };

        // 2N|(Q-1)
        let coeff_modulus: usize = ring_modulus.as_into();
        ...
        let q: u64 = <Q as Field>::MODULUS_VALUE.as_into();
        assert!(t <= q && t.is_power_of_two());
        let lwe_params = LWEParameters {
            dimension: lwe_dimension,
            plain_modulus: params.lwe_plain_modulus,
    ...
    /// Returns the LWE dimension of this [`Parameters<Q>`], refers to **n** in the paper.
    #[inline]
    pub fn lwe_dimension(&self) -> usize {
        self.lwe_params.dimension
    }

    /// Returns the LWE message modulus of this [`Parameters<Q>`], refers to **t** in the paper.
    #[inline]
    pub fn lwe_plain_modulus(&self) -> <Q as Field>::Value {
        self.lwe_params.plain_modulus
    }

    /// Returns the LWE cipher modulus value of this [`Parameters<Q>`], refers to **q** in the paper.
    #[inline]
    pub fn lwe_cipher_modulus_value(&self) -> <Q as Field>::Value {
        <Q as Field>::MODULUS_VALUE
    }

    /// Returns the LWE noise error's standard deviation of this [`Parameters<Q>`].
    #[inline]
    pub fn lwe_noise_standard_deviation(&self) -> f64 {
        self.lwe_params.noise_standard_deviation
    }

    /// Returns the LWE Secret Key distribution Type of this [`Parameters<Q>`].
    #[inline]
    pub fn lwe_secret_key_type(&self) -> LWESecretKeyType {
        self.lwe_params.secret_key_type
    }

    /// Returns the ring dimension of this [`Parameters<Q>`], refers to **N** in the paper.
    #[inline]
    pub fn ring_dimension(&self) -> usize {
        self.blind_rotation_params.dimension
    }

    /// Returns the ring modulus of this [`Parameters<Q>`], refers to **Q** in the paper.
    #[inline]
    pub fn ring_modulus(&self) -> <Q as Field>::Value {
        self.blind_rotation_params.modulus
    }

    /// Returns the ring noise error's standard deviation of this [`Parameters<Q>`].
    #[inline]
    pub fn ring_noise_standard_deviation(&self) -> f64 {
        self.blind_rotation_params.noise_standard_deviation
    }

    /// Returns the ring secret key type of this [`Parameters<Q>`].
    #[inline]
    pub fn ring_secret_key_type(&self) -> RingSecretKeyType {
        self.blind_rotation_params.secret_key_type
    }

    /// Returns the gadget basis of this [`Parameters<Q>`],
    ...
    /// Returns the key switching basis' bits of this [`Parameters<Q>`],
    ...
    /// Returns the key switching error's standard deviation of this [`Parameters<Q>`].
    #[inline]
    ...
    #[inline]
    pub fn lwe_noise_distribution(&self) -> FieldDiscreteGaussianSampler {
        FieldDiscreteGaussianSampler::new(0.0, self.lwe_noise_standard_deviation()).unwrap()
    }
    ...
    /// Returns the LWE parameters of this [`Parameters<Q>`].
    #[inline]
    pub fn lwe_params(&self) -> LWEParameters<Q> {
        self.lwe_params
    }

    /// Returns the process before blind rotation of this [`Parameters<Q>`].
    #[inline]
    pub fn process_before_blind_rotation(&self) -> ProcessBeforeBlindRotation<Q> {
        self.process_before_blind_rotation
    }

    /// Returns the lut step of this [`Parameters<Q>`].
    #[inline]
